<?php
    /**
     * @package POWr Notification Bar
     * @version 1.6.4
     */
    /*
    Plugin Name: POWr Notification Bar
    Plugin URI: https://www.powr.io/plugins/notification-bar
    Description: Engage and direct your visitors with an effective & customizable Notification Bar.  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    class Powr_Notification_Bar extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_notification_bar',
                             __( 'POWr Notification Bar' ),
                             array( 'description' => __( 'Notification Bar by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-notification-bar' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Notification Bar.
        </p>
        <p>
    ...
    function register_powr_notification_bar() {
      register_widget( 'Powr_Notification_Bar' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_notification_bar' );
    //Create short codes for adding plugins anywhere:
    function powr_notification_bar_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-notification-bar' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-notification-bar' label='$label'></div>";
      }else{
      	"<div class='powr-notification-bar'></div>";
      }
    }
    add_shortcode( 'powr-notification-bar', 'powr_notification_bar_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_notification_bar_menu' );
    function powr_notification_bar_menu() {
      add_menu_page( 'POWr Notification Bar', 'POWr Notification Bar', 'manage_options', 'powr-notification-bar-settings', 'powr_notification_bar_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_notification_bar_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/notification-bar';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/notification-bar';
      }
    ...
    register_activation_hook(__FILE__, 'powr_notification_bar_plugin_activate');
      add_action('admin_init', 'powr_notification_bar_plugin_redirect');

      function powr_notification_bar_plugin_activate() {
      add_option('powr_notification_bar_plugin_do_activation_redirect', true);
      }
      ...
      function powr_notification_bar_plugin_redirect() {
      if (get_option('powr_notification_bar_plugin_do_activation_redirect', false)) {
          delete_option('powr_notification_bar_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-notification-bar-settings&' );
          }
       }
      }
